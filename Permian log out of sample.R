...
permian$last_prod_date <- "2014-12-01"

#permian <- dbGetQuery(base, "select * from dev.zsz_permian_out")
...
new_prod <- dbGetQuery(base, "select first_prod_date, extract('year' from first_prod_date) first_prod_year, 
                       ...
                       where liq_cum >0 and prod_date >= '2012-12-01' and ALLOC_PLUS IN ('Y','X') and liq >= 0 and basin = 'PERMIAN BASIN' 
...
hist_prod <- dbGetQuery(base, "select prod_date, round(sum(liq)/1000/(extract(days from (prod_date + interval '1 month' - prod_date))),0) as prod
                        ...
                        and basin = 'PERMIAN BASIN' and prod_date < '2015-01-01'
                        group by prod_date
                        order by 1")
